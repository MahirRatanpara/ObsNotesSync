...
**Neither eliminates the fundamental problem:** a GC pause longer than the session timeout means the lock is released while the holder still believes it holds it. **You still need a fencing token** — ZooKeeper's `zxid` serves as one. See [Two Generals Problem](../../09%20Distributed%20Systems/Theory/Two%20Generals%20Problem.md).
...
- [Consensus Algorithms](../../09%20Distributed%20Systems/Consensus/Consensus%20Algorithms.md)
