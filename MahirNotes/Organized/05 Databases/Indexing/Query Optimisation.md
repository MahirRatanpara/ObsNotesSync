...
**Offset is O(offset); keyset is O(limit).** It's also *correct* under concurrent inserts, where offset pagination repeats or skips rows. See [API Design](../../04%20High%20Level%20Design/Core%20Concepts/API%20Design.md).
...
**Fixes:** `JOIN FETCH`, `@EntityGraph`, batch loading, or a DTO projection. See [JPA and Hibernate](../../14%20Spring%20Boot/JPA%20and%20Hibernate.md).
