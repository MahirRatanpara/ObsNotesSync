...
`@Transactional(isolation = Isolation.REPEATABLE_READ)` maps directly to database isolation levels — see [Transactions and Isolation Levels](../05%20Databases/Consistency%20and%20Transactions/Transactions%20and%20Isolation%20Levels.md). `DEFAULT` uses the database's own default, which differs between Postgres (Read Committed) and MySQL (Repeatable Read).
