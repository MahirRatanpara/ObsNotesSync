...
Every consumer needs an idempotency key. See [Idempotent Consumers](Idempotent%20Consumers.md).
...
**Solution: the outbox pattern** — write the event into an `outbox` table in the same local transaction, and have a relay (or CDC) publish from it. Covered in [Multi-Step Processes and Saga](Multi%20Step%20Processes%20and%20Saga.md).
